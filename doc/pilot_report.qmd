...
author:
   ...
   - name: Leeanne McGurk
     affiliations: Medical Sciences Institute
     note: "Collaborator"
date: today
...
Reads were mapped to the *Drosophila melanogaster* genome BDGP6.32 (release 107), downloaded from Ensembl. The provided 28S sequence was added as a separate 'chromosome' (here labelled as `pdb|3J3E|5`). All `C` nucleotides were converted to `T` before mapping, to account for bisulphite convertion in data. *Bowtie2* 2.4.5 was used for mapping with default parameters.
...
A few regions with at least reads at any position were found. They are shown in @fig-regions for sample `CTR_1`.
...
All these regions contain the same sequence.
...
#| fig-cap: Coverage of 28S. The sequence of the region between the primers is shown. Depth drops sharply outside primer regions.
...
*Bismark* 0.23.1 was used to detect methylation sites. There are several sites in CpG context in 28S, though non of them is methylated except the site of interest, 3402, which is highly methylated in all samples. @fig-28S-methylation illustrates methylation for all sites detecte, @tbl-3402-methylation shows methylation statistics for locus 3402.


```{r fig_methyl}
#| label: fig-28S-methylation
#| fig-cap: Methylation sites in 28S. The colous show counts of reads with and without methylation.
#| fig-width: 10
#| fig-height: 10
