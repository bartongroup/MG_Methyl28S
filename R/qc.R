...
plot_qualities <- function(qc) {
  qc %>%
    ggplot(aes(x=Base, y=Mean, group=root)) +
    theme_bw() +
    geom_line(alpha=0.1) +
...
parse_one_bamstat <- function(file) {
  if (!file.exists(file)) {
    warning(paste("File", file, "not found"))
    return(NULL)
  }
  tibble(x = read_lines(file, skip = 5)) |> 
    filter(x != "") |> 
...
parse_bamstats <- function(path, meta) {
  map2_dfr(meta$raw_sample, meta$sample, function(rsam, sam) {
    parse_one_bamstat(file.path(path, paste0(rsam, ".stats"))) |> 
      add_column(sample = sam)
  })
}

tabulate_bamstats <- function(bs) {
  bs |> 
